# =============================================================================
# Packer Template: Kali Linux (Purple) 2025.4
# =============================================================================
# Builds a Proxmox VM template for Kali Linux Purple 2025.4 with:
#   - VirtIO drivers (in-kernel)
#   - QEMU Guest Agent
#   - SPICE agent (spice-vdagent)
#   - kali-linux-purple metapackage (defensive security tools)
...
variable "iso_file" {
  type        = string
  description = "Proxmox storage path to Kali Linux Purple 2025.4 ISO"
}
...
variable "template_name" {
  type    = string
  default = "cdx-kali-purple-base"
}

source "proxmox-iso" "kali-purple-2025-4" {
  # Proxmox connection
  ...
  boot_command = [
    ...
    "auto=true ",
    "priority=critical ",
    "preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/preseed/kali-purple/preseed.cfg ",
    "locale=en_US.UTF-8 ",
    "keymap=us ",
    "hostname=kali-purple-template ",
    "domain=cdx.lab ",
    "--- <enter>"
  ]
...
build {
  sources = ["source.proxmox-iso.kali-purple-2025-4"]
