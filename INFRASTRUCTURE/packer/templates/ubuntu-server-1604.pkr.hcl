# =============================================================================
# Packer Template: Ubuntu Server 16.04
# =============================================================================
# Builds a Proxmox VM template for Ubuntu Server 16.04 with:
#   - VirtIO drivers (in-kernel)
#   - QEMU Guest Agent
#   - SPICE agent (spice-vdagent)
#   - CDX base config (cdxadmin account, SSH)
#
# Note: Ubuntu 16.04 uses the legacy Debian installer (preseed), NOT autoinstall.
# =============================================================================
...
variable "iso_file" {
  type        = string
  description = "Proxmox storage path to Ubuntu Server 16.04 ISO"
}
...
variable "template_name" {
  type    = string
  default = "cdx-ubuntu1604-base"
}

source "proxmox-iso" "ubuntu-server-1604" {
  # Proxmox connection
  ...
  # Preseed via Packer HTTP server (legacy Debian installer)
  http_directory = "../http"
  boot_wait      = "5s"
  boot_command = [
    "<enter><wait><f6><esc>",
    ...
    "auto=true ",
    "priority=critical ",
    "url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/preseed/ubuntu-1604/preseed.cfg ",
    "locale=en_US.UTF-8 ",
    "kbd-chooser/method=us ",
    "hostname=ubuntu1604-template ",
    "domain=cdx.lab ",
...
build {
  sources = ["source.proxmox-iso.ubuntu-server-1604"]
