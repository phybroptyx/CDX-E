# =============================================================================
# Packer Template: Windows 8.1
# =============================================================================
# Builds a Proxmox VM template for Windows 8.1 with:
#   - VirtIO drivers (SCSI, network, balloon) — virtio-win 0.1.185
#   - QEMU Guest Agent
...
variable "iso_file" {
  type        = string
  description = "Proxmox storage path to Windows 8.1 ISO"
}

variable "virtio_iso_file" {
  type        = string
  description = "Proxmox storage path to VirtIO-Win ISO (0.1.185 for Windows 8.1)"
}
...
variable "template_name" {
  type    = string
  default = "cdx-win81-base"
}

source "proxmox-iso" "windows-81" {
  # Proxmox connection
  ...
  additional_iso_files {
    device           = "sata2"
    cd_files         = ["../http/autounattend/win81/autounattend.xml"]
    cd_label         = "OEMDRV"
  ...
  sockets  = 1
  cpu_type = "host"
  os       = "win8"  # Proxmox OS type for Windows 8.1
...
build {
  sources = ["source.proxmox-iso.windows-81"]
