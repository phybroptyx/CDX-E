# =============================================================================
# CDX-E Packer Template: FLARE VM
# =============================================================================
# Builds the FLARE VM malware analysis and reverse engineering toolkit on
# Proxmox VE. Uses the proxmox-clone source — clones the Windows 10 base
# template (VMID 2035, cdx-win10-base) and installs the FLARE VM toolset via
...
#   - Windows 10 Pro with FLARE VM toolset installed
#   - All base template configuration (VirtIO drivers, QEMU GA, WinRM) intact
#   - Malware analysis tools installed (IDA, Ghidra, x64dbg, etc.)
...
#     -var-file=../vars/flare_vm.pkrvars.hcl \
...
#
# Timing notes:
#   FLARE VM installs hundreds of packages via Chocolatey. The provisioner
...
variable "vm_id" {
  type        = number
  default     = 2049
  description = "Proxmox VMID for the new FLARE VM template."
}

variable "vm_name" {
  type        = string
  default     = "cdx-flarevm-base"
  description = "Proxmox VM/template name. Must match template_registry.flare_vm.proxmox_name."
}
...
variable "disk_size" {
  type        = string
  default     = "80G"
  description = "Root disk size. 80G accommodates the large FLARE VM tool set."
}
...
source "proxmox-clone" "flare_vm" {
  ...
  template_description = "CDX FLARE VM (Win10 base + malware analysis toolset) — built by Packer on ${timestamp()}"
  ...
  # accommodate the FLARE VM tool chain.
  disks {
  ...
  # ── WinRM communicator ──────────────────────────────────────────────────────
  # Extended timeout (2h) — FLARE VM installs 100+ packages via Chocolatey.
  communicator   = "winrm"
...
# =============================================================================
build {
  sources = ["source.proxmox-clone.flare_vm"]

  # ── FLARE VM installation ─────────────────────────────────────────────────────
  # Downloads and runs the official FLARE VM installer from the Mandiant
  # GitHub repository. The installer uses Chocolatey to deploy tools.
  # The CDX-I virtual internet provides outbound connectivity for downloads.
  provisioner "powershell" {
    elevated_user     = var.winrm_username
    elevated_password = var.winrm_password
    inline = [
      "Write-Host '=== CDX-E FLARE VM installation starting ==='",

      # Set execution policy
      "Set-ExecutionPolicy Unrestricted -Scope LocalMachine -Force",

      # Download the FLARE VM installer
      "$installerUrl = 'https://raw.githubusercontent.com/mandiant/flare-vm/main/install.ps1'",
      "$installerPath = 'C:\\Windows\\Temp\\flare-install.ps1'",
      "Invoke-WebRequest -Uri $installerUrl -OutFile $installerPath -UseBasicParsing",

      # Run installer non-interactively
      "& $installerPath -noWait -noGui -noChecks",

      "Write-Host '=== FLARE VM installation complete ==='",
    ]
  }
  ...
  provisioner "powershell" {
    inline = [
      "Write-Host '=== CDX-E FLARE VM post-install check ==='",
