# =============================================================================
# Packer Template: Windows Server 2012 R2
# =============================================================================
# Builds a Proxmox VM template for Windows Server 2012 R2 with:
#   - VirtIO drivers (SCSI, network, balloon)
#   - QEMU Guest Agent
...
variable "iso_file" {
  type        = string
  description = "Proxmox storage path to Windows Server 2012 R2 ISO"
}

variable "virtio_iso_file" {
  type        = string
  description = "Proxmox storage path to VirtIO-Win ISO"
}
...
variable "template_name" {
  type    = string
  default = "cdx-win2012r2-base"
}

variable "autounattend_file" {
  type    = string
  default = "../http/autounattend/win2012r2/autounattend.xml"
}

source "proxmox-iso" "windows-server-2012r2" {
  # Proxmox connection
  ...
  additional_iso_files {
    device           = "sata2"
    cd_files         = [var.autounattend_file]
    cd_label         = "OEMDRV"
  ...
  sockets  = 1
  cpu_type = "host"
  os       = "win8"  # Proxmox OS type for 2012 R2
  ...
  # Unattended install - Packer HTTP server serves autounattend.xml
  http_directory = "../http"
  boot_wait      = "5s"
...
build {
  sources = ["source.proxmox-iso.windows-server-2012r2"]
