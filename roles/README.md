...
- [Operational](#operational) — agents, validation, cluster bootstrap, checkpoint, readiness, snapshot
...
(e.g., `routers,domain_controllers` for Phase 2b, `workstations` for Phase 6a).
...
**Called from:** `server_configuration.yml` (in `roles:` list, after `install_windowsfeature`),
`workstation_configuration.yml` (in `roles:` list, before `install_windowsfeature`)

Joins a Windows VM to the exercise Active Directory domain. Placed in the `roles:` list
(never in `post_tasks`) so it cannot be silently skipped if an earlier role calls
`meta: end_host`. `configure_vm` runs first to set the hostname (and reboot if needed)
while the machine is still in workgroup — renaming an already domain-joined computer
requires domain admin rights and would fail.
...
**Called from:** `domain_services.yml`, `dhcp_management.yml` (Phase 4a / Phase 7)
...
**Called from:** `domain_management.yml`, `server_configuration.yml`, `workstation_configuration.yml`,
`domain_services.yml`, `post_deployment.yml`

Provides phase state tracking so that completed pipeline phases are not re-executed on
resume. State is persisted to `EXERCISES/<name>/phase_state.yml` (runtime artifact,
gitignored).
...
| Action | Behavior |
|---|---|
| `check` | Reads `phase_state.yml`; exits via `meta: end_play` if the target checkpoint key is already marked `complete` |
| `record_complete` | Writes `<phase_key>: complete` to `phase_state.yml` |
| `record_failed` | Writes `<phase_key>: failed` to `phase_state.yml` |
| `reset` | Deletes `phase_state.yml` entirely (use with `reset_checkpoint_state=true`) |

**Key variables:**

| Variable | Description |
|---|---|
| `pipeline_checkpoint_action` | `check`, `record_complete`, `record_failed`, or `reset` |
| `pipeline_checkpoint_phase` | Checkpoint key string (e.g., `phase4a_dhcp`, `phase6`) |
| `reset_checkpoint_state` | Set `true` at runtime to force all phases to re-run regardless of checkpoint |

---

### `connectivity_test`

**Called from:** `post_deployment.yml` (Phase 11)

Validates that all exercise VMs are reachable via Proxmox guest-exec after deployment.
Tests are performed entirely through the QEMU guest agent API — no WinRM or SSH dependency.
Any test failure causes the play to fail, blocking the subsequent `snapshot_exercise` role
to prevent a broken deployment from being snapshotted.

---

### `snapshot_exercise`

**Called from:** `post_deployment.yml` (Phase 11, after `connectivity_test`)

Shuts down all exercise VMs in dependency order (workstations → servers → DCs → routers)
and creates a Proxmox VM snapshot for each. The snapshot serves as a clean restore point
for exercise resets between exercise runs. Groups listed in `snapshot_skip_groups` are
excluded (e.g., `['routers']` for exercises where routers should remain running).

**Key variables:**

| Variable | Default | Description |
|---|---|---|
| `snapshot_name` | `pre_exercise` | Name applied to all VM snapshots |
| `snapshot_include_ram` | `false` | Include live RAM state in snapshot (requires VM to be running) |
| `snapshot_skip_groups` | `[]` | List of `ansible_group` values to skip during shutdown and snapshot |

---

### `vm_readiness`

**Called from:** `vm_power_management` (via include_role), other roles requiring per-VM readiness checks

Implements a per-VM readiness ladder using progressively more capable probes. Prevents
subsequent configuration tasks from running against a VM that has not completed boot.

**Probe sequence (stops at first passing probe):**

1. QEMU guest agent ping (`/agent/ping`)
2. Port probe: WinRM 5985 (Windows), SSH 22 (Linux/VyOS), or HTTP (monitoring stacks)
3. WinRM `win_ping` auth-level check (Windows only)

Probe type is determined by `ansible_os_type` and VM-level `readiness_probe` override
in `vms.yaml`.
...
**Called from:** `configure_vm` (Windows branch, when `windows_kms_server` is defined)

Activates Windows against a KMS server. Runs `slmgr.vbs /skms <server>` then
`slmgr.vbs /ato`. Skips silently if `windows_kms_server` is not set in `all.yml`
or `scenario.yml`.
...
**Called from:** `domain_management.yml` (after `configure_active_directory`, when
`prestage_computers: true` is set in the exercise configuration)

Creates AD computer objects in the target OU before member servers or workstations are
joined. Pre-staging allows domain join to succeed without requiring the joining account
to have Create Computer Objects permission in the default `Computers` container.
